...
from Autodesk.Revit.DB import (
    ...
    Transaction, TransactionGroup, FamilySymbol, ViewDuplicateOption,
...
# before wrapping up. Raised from 8 once code execution + tool search
# landed: legitimate multi-step jobs (create N sheets + duplicate N views
# + apply templates + place them) genuinely need many rounds, especially
# when the model works one item at a time. Still a backstop against a
# runaway loop; the user can also hit Stop. The model is told to batch
# multi-step work into a single exec_revit_python call to stay well under
# this.
MAX_AGENT_ROUNDS = 25
...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "\n"
    "TOOLS ARE SEARCHABLE, not all preloaded. You always have exec_revit_python, "
    "search_revit_api + revit_api_lookup (API grounding), and a few core reads "
    "(get_document_info, get_selection, get_element_details, list_views, list_sheets, "
    "count_by_category). Everything else "
    "(creating sheets, tagging, schedules, exports, spaces, MEP systems, revisions, view "
    "setup, links, placement, etc.) is a DEDICATED tool you find with the tool-search tool. "
    "When a request likely has a purpose-built tool, SEARCH for it first (query by intent, "
    "e.g. 'create sheet', 'tag elements', 'export pdf', 'create spaces from link') and prefer "
    "it - dedicated tools carry validation, confirmation dialogs, and clean undo. Use "
    "exec_revit_python for the long tail nothing covers.\n"
    "\n"
    ...
    "  - GROUND YOUR CODE (search -> lookup -> exec). The Revit API changes across versions "
    "and you WILL misremember signatures. If you don't know WHICH class/method you need, "
    "search_revit_api(query) to discover it (e.g. 'duct', 'view template', 'bounding box'). "
    "Then revit_api_lookup(type_name) for the exact methods/signatures in THIS version. Then "
    "write the code. You can also introspect live in a read (result = dir(element)). Verifying "
    "beats guessing.\n"
    ...
    "  - BATCH MULTI-STEP WORK INTO ONE exec_revit_python CALL. When a request has several "
    "steps (e.g. create 3 sheets + duplicate 3 views + apply a template + place each view on "
    "its sheet), write ONE code block that does the WHOLE sequence in a loop, rather than many "
    "separate tool calls. It's faster, it's atomic (one undo), and a turn is limited to ~25 "
    "tool rounds - doing 12 things as 12 calls can hit that limit and stall. One code block "
    "doing 12 things is a single round. Look up any unfamiliar API with revit_api_lookup "
    "first, then write the full script.\n"
    "\n"
    ...
    "the preamble already identifies the subject. Image AND PDF attachments are real - you "
    "can see/read them directly: describe images, read text + tables + numbers out of PDFs "
    "(spec sections, cut sheets, code letters, RFIs). You can combine a document with the "
    "model - e.g. read a cut sheet's CFM/model number and set_element_parameters on the "
    "matching equipment, or compare a spec PDF against a schedule. Don't claim you can't open "
    "a PDF that's attached.\n"
    "\n"
    ...
    "\n"
    "13b. WEB SEARCH. You have a web_search tool. USE IT for anything where being current + "
    "exact matters and your training data could be stale or wrong: specific ASHRAE / IECC / "
    "NEC / IMC / IPC / NFPA code values + table numbers + edition-specific requirements, code "
    "adoption by jurisdiction, manufacturer product data (Trane / Carrier / Daikin / etc. "
    "model specs), and anything time-sensitive. A confidently-wrong code value is dangerous - "
    "when you're not certain of an exact number or which edition applies, SEARCH rather than "
    "recite from memory, and cite the standard + edition. Don't search for basic, stable "
    "engineering knowledge you reliably know (it adds latency + cost). The UI shows your "
    "sources automatically, so the user can verify.\n"
    "\n"
...
# Marker that the CURRENT built-in prompt always contains. Used to detect
# a legacy full-replacement system_prompt frozen in an old config (those
# predate code execution and never mention it). Such a stored prompt
# silently overrode every prompt update we shipped - so we drop it.
_PROMPT_CAPABILITY_MARKER = "exec_revit_python"
...
def load_config():
    ...
    # System prompt model (changed): the built-in DEFAULT_SYSTEM_PROMPT is
    # now ALWAYS the base and can't be replaced - that base carries all
    # the capability + tool-search guidance the model needs. The config
    # stores only `system_prompt_extra`: optional firm-specific additions
    # appended to the base.
    #
    # MIGRATION: older configs stored a full `system_prompt` that REPLACED
    # the default. Those froze a stale prompt (e.g. the v1 ASHRAE-only one
    # that never mentioned tools) and silently overrode every update we
    # shipped. On load: if a legacy full prompt is present, keep it as an
    # addendum ONLY if it looks user-authored; otherwise drop it. We can't
    # perfectly tell, so the safe rule is: drop any legacy prompt that
    # lacks the current capability marker (it's a stale default, not a
    # deliberate customization). A genuinely custom prompt the user wrote
    # would be re-entered via Settings as an addendum.
    extra = raw.get("system_prompt_extra")
    if extra is None:
        # Legacy `system_prompt` (full replacement) is intentionally NOT
        # carried over - those are stale defaults that silently overrode
        # every prompt update. The built-in base now covers everything.
        extra = ""
    ...
    return {"api_key":             api_key,
            "model_key":           model_key,
            "system_prompt_extra": extra or "",
            "spend_threshold":     spend_threshold}


def save_config(api_key, model_key, system_prompt_extra, spend_threshold=None):
...
def _effective_system_prompt(config):
    """The actual system prompt sent to the API: the built-in base
    (always current, never overridden) plus any user addendum from
    Settings. This guarantees capability + tool-search guidance always
    reaches the model regardless of what's saved in config."""
    base = DEFAULT_SYSTEM_PROMPT
    extra = ((config or {}).get("system_prompt_extra") or "").strip()
    if extra:
        return base + "\n\nFIRM-SPECIFIC ADDITIONAL INSTRUCTIONS:\n" + extra
    return base
...
# Lazily-built, session-cached index of the live Revit API surface.
# Shape: {"types": [(short, full), ...], "members": [(member, short), ...]}
# Built once (first search) by reflecting the loaded RevitAPI +
# RevitAPIUI assemblies. The build is a few seconds the first time; every
# search after is an instant in-memory scan.
_REVIT_API_INDEX = None


def _build_revit_api_index():
    """Reflect the loaded Revit assemblies into a searchable index of
    public type names + member names. Cached for the session. Defensive
    throughout - reflection can throw on individual types."""
    global _REVIT_API_INDEX
    if _REVIT_API_INDEX is not None:
        return _REVIT_API_INDEX

    import System.Reflection as _Refl

    # Seed assemblies from a known type in each namespace we care about.
    asms = []
    seen = set()
    seed_imports = [
    ...
    for mod_name, type_name in seed_imports:
        try:
            mod = __import__(mod_name, fromlist=[type_name])
            seed = getattr(mod, type_name, None)
            if seed is None:
                continue
            a = clr.GetClrType(seed).Assembly
            if a.FullName not in seen:
                seen.add(a.FullName)
                asms.append(a)
        except Exception:
            continue

    flags = (_Refl.BindingFlags.Public | _Refl.BindingFlags.Instance |
             _Refl.BindingFlags.Static | _Refl.BindingFlags.DeclaredOnly)

    types_out = []
    members_out = []
    MEMBER_CAP = 250000  # hard backstop against pathological memory use
    for a in asms:
        try:
            tlist = a.GetExportedTypes()
        except Exception:
            continue
        for t in tlist:
            try:
                short = t.Name
                full = t.FullName
                types_out.append((short, full))
                if len(members_out) >= MEMBER_CAP:
                    continue
                for m in t.GetMembers(flags):
                    try:
                        nm = m.Name
                    except Exception:
                        continue
                    # Skip property/event accessor + operator noise.
                    if (nm.startswith("get_") or nm.startswith("set_") or
                            nm.startswith("add_") or nm.startswith("remove_") or
                            nm.startswith("op_") or nm.startswith(".")):
                        continue
                    members_out.append((nm, short))
                    if len(members_out) >= MEMBER_CAP:
                        break
            except Exception:
                continue

    _REVIT_API_INDEX = {"types": types_out, "members": members_out}
    return _REVIT_API_INDEX


def _tool_search_revit_api(doc, input_dict):
    """Keyword-search the live Revit API to DISCOVER the right class or
    method when you don't already know its name. Returns matching type
    names + 'Type.Member' hits. The discovery half of grounding: use it
    to find what to call, then revit_api_lookup(type) for exact
    signatures, then exec_revit_python to do it.

    e.g. query 'duct' -> Duct, DuctType, Duct.Create, ...;
         query 'view template' -> View.CreateViewTemplate, ...;
         query 'bounding box' -> Element.get_BoundingBox, BoundingBoxXYZ."""
    inp = input_dict or {}
    q = (inp.get("query") or "").strip().lower()
    max_results = int(inp.get("max_results") or 40)
    if not q:
        return {"error": "query is required (a keyword like 'duct', "
                         "'view template', 'create wall')."}

    try:
        idx = _build_revit_api_index()
    except Exception as e:
        return {"error": "Could not build API index: {}".format(e)}

    # Type-name matches (short or full name contains the query).
    type_hits = []
    seen_types = set()
    for short, full in idx["types"]:
        try:
            if q in short.lower() or q in full.lower():
                if full not in seen_types:
                    seen_types.add(full)
                    type_hits.append(full)
        except Exception:
            continue

    # Member-name matches -> "TypeShort.Member".
    member_hits = []
    seen_members = set()
    for nm, short in idx["members"]:
        try:
            if q in nm.lower():
                key = "{}.{}".format(short, nm)
    ...
    # Sort: exact/short names first, then alphabetical. Keep it simple +
    # cap to protect the token budget.
    type_hits = sorted(set(type_hits))[:max_results]
    member_hits = sorted(set(member_hits))[:max_results]

    out = {
        "query":         q,
        "type_matches":  type_hits,
        "member_matches": member_hits,
        "type_count":    len(type_hits),
        "member_count":  len(member_hits),
    }
    if not type_hits and not member_hits:
        out["note"] = ("No matches. Try a different keyword (Revit type "
                       "names are descriptive: 'Duct', 'Pipe', 'Wall', "
                       "'View', 'Sheet', 'Level', 'Room', 'Space').")
    return out
...
TOOL_DEFS = [
    ...
    },
    {
        ...
        "description": (
            "Keyword-search the live Revit API to DISCOVER the right "
            "class or method when you don't know its name. Returns "
            "matching type names + 'Type.Member' hits. This is the "
            "discovery step: search_revit_api(query) to find what to "
            "call, then revit_api_lookup(type) for the exact "
            "signatures, then exec_revit_python to do it. Examples: "
            "query 'duct' finds Duct/DuctType/Duct.Create; 'view "
            "template' finds View.CreateViewTemplate; 'bounding box' "
            "finds Element.get_BoundingBox + BoundingBoxXYZ."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
...
TOOL_IMPLS = {
    ...
    "search_revit_api":          _tool_search_revit_api,
...
def _attachment_short_label(att):
    ...
    if kind == "pdf":
        return att.get("name") or "document.pdf"
    return "Attachment"
...
def _attachment_full_label(att):
    ...
    if kind == "pdf":
        bits = ["name={}".format(att.get("name") or "document.pdf")]
        if att.get("pages"):
            bits.append("~{} pages".format(att["pages"]))
        if att.get("byte_size") is not None:
            bits.append("size={}".format(_human_size(att["byte_size"])))
        return "PDF: " + ", ".join(bits)
    return repr(att)
...
def _build_attachment_preamble(attachments):
    ...
    Image AND pdf attachments are skipped here - they're sent as
    separate `image` / `document` content blocks in the API call (see
    _build_api_messages), so listing them in a text preamble would be
    redundant."""
    if not attachments:
        return ""
    # Filter out images + pdfs for preamble purposes - they're sent as
    # their own content blocks alongside this text.
    text_attachments = [a for a in attachments
                        if a.get("kind") not in ("image", "pdf")]
...
# PDF documents: sent to the API as `document` content blocks (base64),
# the same pattern as images. Anthropic caps a single request at 32 MB
# total; we cap the PDF itself a bit below that to leave room for the
# rest of the payload. Bigger PDFs fall through to a metadata-only file
# chip so Claude at least sees the filename.
_PDF_MEDIA_TYPE = "application/pdf"
_PDF_MAX_BYTES  = 28 * 1024 * 1024
...
def _attachment_from_pdf_path(path):
    """Read a PDF from disk and return a 'pdf' attachment (base64 data
    for a document content block). Returns None if missing, not a PDF,
    or too large for one request."""
    if not path or not os.path.isfile(path):
        return None
    if not _is_pdf_ext(os.path.splitext(path)[1]):
        return None
    try:
        size = os.path.getsize(path)
    except Exception:
        size = None
    if size is not None and size > _PDF_MAX_BYTES:
        return None  # too big -> caller falls through to a file chip
    try:
        raw_bytes = NetFile.ReadAllBytes(path)
        b64 = Convert.ToBase64String(raw_bytes)
    except Exception:
        return None
    # Best-effort page count: scan for PDF page markers. Cheap, not
    # exact (form XObjects can inflate it), but good enough for display
    # + warning the model about big docs.
    pages = None
    try:
        import System.Text as _Text
        head = raw_bytes
        text = _Text.Encoding.Latin1.GetString(head) if hasattr(_Text.Encoding, "Latin1") \
            else _Text.Encoding.GetEncoding(28591).GetString(head)
        pages = text.count("/Type/Page") + text.count("/Type /Page")
        if pages <= 0:
            pages = None
    except Exception:
        pages = None
...
def _attachment_from_path(path):
    ...
    try:
        ...
        # PDFs become document attachments (Claude reads them directly).
        if _is_pdf_ext(ext):
            pdf_att = _attachment_from_pdf_path(path)
            if pdf_att is not None:
                return pdf_att
            # Too big / unreadable -> fall through to a file chip so the
            # filename is at least visible to Claude.
...
def _collect_citations(blocks):
    """Pull unique web-search citations (url + title) out of a round's
    text blocks. Returns a list of {url, title} dicts, deduped by URL."""
    out = []
    seen = set()
    for b in blocks or []:
        ...
        for c in (b.get("citations") or []):
            ...
            if not url or url in seen:
                continue
            seen.add(url)
            out.append({"url": url, "title": c.get("title") or url})
    return out
...
def summarize_tool_result(result):
    ...
    if isinstance(result, dict):
        ...
        if "type_matches" in result and "member_matches" in result:
            return "API search: {} types, {} members".format(
                result.get("type_count", 0), result.get("member_count", 0))
...
def _compact_tool_for_api(tool_def, max_description_chars=240):
    ...
    - Server-side tools (tool search, web search, etc.) carry a `type`
      field and no input_schema; they pass through unchanged."""
    if tool_def.get("type"):
...
# Tools kept always-loaded (non-deferred). Everything else is deferred
# and discovered on demand via the tool-search tool. Keeping this set
# small is the whole point: it's what keeps the per-request token count
# (and the 30k/min rate limit) low. exec_revit_python + revit_api_lookup
# stay loaded because they're the universal escape hatch + its grounding
# companion; the rest are the highest-frequency reads.
CORE_TOOL_NAMES = set([
    "exec_revit_python",
    "revit_api_lookup",
    "search_revit_api",
...
# Server-side tool-search tool. BM25 (natural-language queries) is more
# robust for us than the regex variant (no Claude-generated regex to go
# wrong). GA - no anthropic-beta header needed.
TOOL_SEARCH_TOOL_DEF = {
    "type": "tool_search_tool_bm25_20251119",
    "name": "tool_search_tool_bm25",
}

# Server-side web search (GA, no beta header). Lets Claude look up
# current/exact ASHRAE / IECC / NEC code values, manufacturer specs,
# and anything its training data might have wrong or stale - and cite
# sources. max_uses caps searches per request (cost + latency control).
WEB_SEARCH_TOOL_DEF = {
...
def _build_api_tools(tools):
    """Build the tools array for the request with deferred loading.

    Layout: [web_search, tool_search_tool, <core tools>, <deferred tools>].
      - The server-side tools (web search, tool search) + core tools
        are always loaded.
      - Everything else is marked defer_loading=true: its full
        definition is still in the payload (so historical tool_use
        blocks validate), but it does NOT count toward input tokens
        until Claude searches for and discovers it. This is what
        collapses the ~190-tool token bill that was blowing the rate
        limit.
      - cache_control goes on the LAST non-deferred (core) tool so the
        cached prefix covers exactly the always-loaded set; deferred
        tools sit after the breakpoint and load on demand into the
        message history, never invalidating the prefix cache."""
    search_tool = dict(TOOL_SEARCH_TOOL_DEF)
    web_tool    = dict(WEB_SEARCH_TOOL_DEF)
    core = []
    deferred = []
    for t in (tools or []):
        compact = _compact_tool_for_api(t)
        if t.get("name") in CORE_TOOL_NAMES:
            core.append(compact)
        else:
            compact["defer_loading"] = True
            deferred.append(compact)
    if core:
        core[-1]["cache_control"] = {"type": "ephemeral"}
    else:
        search_tool["cache_control"] = {"type": "ephemeral"}
    return [web_tool, search_tool] + core + deferred
...
def _build_api_messages(history_messages, history_window=HISTORY_WINDOW):
    ...
    for m in trimmed:
        ...
        if m.get("role") == "user" and isinstance(c, str):
            ...
            media = [a for a in atts
                     if isinstance(a, dict)
                     and a.get("data_b64")
                     and a.get("kind") in ("image", "pdf")]
            if media:
                content_list = []
                # Documents (PDFs) first, then images, then the text
                # question - the ordering Anthropic recommends (ingest
                # the document, then process the question).
                for a in media:
                    if a.get("kind") == "pdf":
                        content_list.append({
                            "type": "document",
                ...
                # request valid for media-only sends.
                content_list.append({
                    "type": "text",
                    "text": c if (c or "").strip() else "(see attached document/image)",
                })
                c = content_list
    ...
    # Strip old image + PDF data from the history. Media blocks are big
    # (a screenshot ~1.5-2k tokens, a PDF far more); without this, every
    # long chat re-ships them all on every turn. Keeps only the most
    # recent image-bearing and document-bearing messages intact.
    _strip_old_media(out)
...
def _strip_old_media(messages):
    """In-place: drop image + document (PDF) content blocks from older
    messages, keeping ONLY the most-recent message that carries each
    media type intact. Media blocks are big (a screenshot is ~1.5-2k
    tokens; a PDF far more), so without this every long chat re-ships
    them all on every turn. The most recent image-bearing message and
    the most recent document-bearing message both stay full fidelity so
    the current question can still reference a just-attached file."""
    keep = {"image": -1, "document": -1}
    for i in range(len(messages) - 1, -1, -1):
        c = messages[i].get("content")
        if not isinstance(c, list):
            continue
        for mt in ("image", "document"):
            if keep[mt] == -1 and any(
                    isinstance(b, dict) and b.get("type") == mt for b in c):
                keep[mt] = i
        if keep["image"] != -1 and keep["document"] != -1:
            break
    ...
    for i, msg in enumerate(messages):
        ...
        for b in c:
            ...
            if bt in ("image", "document") and i != keep.get(bt, -1):
                dropped.add("image" if bt == "image" else "document")
                continue
            new_blocks.append(b)
        if dropped:
            label = " + ".join(
                ("PDF" if d == "document" else "image") for d in sorted(dropped))
            new_blocks.insert(0, {
                "type": "text",
                "text": "[{} attached earlier in this conversation, omitted "
                        "from the current request to save tokens - ask the "
                        "user to re-attach if you need it again]".format(label),
            })
...
def _post_stream_round(api_key, model_id, system_prompt, tools, messages,
                       ...
                       on_error, is_cancelled, include_tools=True):
    ...
    # Build the tools array with deferred loading (Tool Search). Only a
    # small core set + the tool-search tool load up-front; the other
    # ~180 tools are deferred and cost zero input tokens until Claude
    # searches for one. This is THE fix for the 30k-tokens/min rate
    # limit - shipping all 190 verbose defs on every request was the
    # cause. _build_api_tools also compacts each def + places the
    # cache_control breakpoint on the last always-loaded tool.
    body_obj = {
        "model":      model_id,
    ...
    # include_tools=False is used by the final wrap-up round when the
    # agent hit the round cap: omitting tools entirely forces the model
    # to produce a text summary instead of starting another tool chain.
    if include_tools:
        body_obj["tools"] = _build_api_tools(tools)
        ...
        try:
            while True:
                ...
                if etype == "content_block_start":
                    ...
                    if btype == "text":
                        current["text"] = ""
                        current["citations"] = []   # web-search source refs
                    elif btype == "tool_use":
                    ...
                    if dtype == "text_delta":
                        ...
                        # Web-search citation attached to the current
                        # text block. Collect for the "Sources" footer.
                        cit = delta.get("citation") or {}
                        if current is not None and current.get("type") == "text":
                            current.setdefault("citations", []).append(cit)
...
class SettingsForm(forms.WPFWindow):
    def __init__(self, current_config):
        ...
        # The box now holds OPTIONAL firm-specific additions appended to
        # the built-in assistant rules (the built-in base is always sent
        # and can't be edited away - that's what carries the tool +
        # capability guidance). Empty by default.
        self.txt_system_prompt.Text = current_config.get(
            "system_prompt_extra", "") or ""
    ...
    def _on_save(self, sender, args):
        ...
        # Optional addendum (may be empty - the built-in base is always sent).
        system_prompt_extra = (self.txt_system_prompt.Text or "").strip()
        ...
        try:
            save_config(api_key, model_key, system_prompt_extra, spend_threshold)
        except Exception as e:
            self.bnr_key_error.Visibility = Visibility.Visible
    ...
    def _on_reset_prompt(self, sender, args):
        # The addendum box resets to EMPTY now (the built-in base is not
        # editable here - it's always applied).
        self.txt_system_prompt.Text = ""
...
class ChatbotForm(forms.WPFWindow):
    ...
    def __init__(self, telemetry_session=None):
        ...
        for border, handler in (
            ...
            (self.popup_attach_file,    self._on_add_attach_file),
    ...
    def _on_add_attach_file(self, sender, args):
        """Open a file picker and attach the chosen file(s). PDFs become
        readable document attachments, images become vision attachments,
        text files inline their content, everything else attaches as a
        metadata chip. Same routing as drag-drop, just a discoverable
        entry point."""
        try:
            from Microsoft.Win32 import OpenFileDialog
        except Exception as e:
            dbhms_ui.info("Could not open file picker: {}".format(e),
                          title="Chatbot")
            return
        dlg = OpenFileDialog()
        dlg.Title = "Attach a file"
        dlg.Multiselect = True
        dlg.Filter = ("Documents + images (*.pdf;*.png;*.jpg;*.jpeg;*.gif;*.webp)"
        ...
        if not ok:
            return
        paths = list(dlg.FileNames) if dlg.FileNames is not None else []
        added = 0
        skipped = []
        for p in paths:
            att = _attachment_from_path(p)
            if att is None:
        ...
        if added:
            self._render_attachments()
            self._set_status("Attached {} file{}.".format(
                added, "" if added == 1 else "s"))
        if skipped:
            dbhms_ui.info(
                "Couldn't attach: {}.\n\nPDFs over 28 MB and unreadable "
                "files are skipped.".format(", ".join(skipped)),
                title="Chatbot")
    ...
    def _render_chat_pane(self):
        ...
        self._turn_citation_urls = set()
        self._turn_sources_panel = None
    ...
    def _ensure_turn_bubble(self):
        ...
        self._turn_openables = []
        self._turn_openable_ids_seen = set()
        # Web-search sources for this turn: one "Sources" panel, deduped
        # by URL, appended/updated as citations stream in.
        self._turn_citation_urls = set()
        self._turn_sources_panel = None
        # Tool-pill collapse state, tracked PER BATCH. A "batch" is a
        # run of consecutive tool calls with no intervening text - the
    ...
    def _do_send(self):
        ...
        if msg_attachments_snapshot:
            ...
            has_images = any(a.get("kind") in ("image", "pdf")
                             for a in msg_attachments_snapshot)
            if preamble:
                api_content = preamble + "\n\n" + (
                    user_text or "(no question text - just providing the context above)")
            elif has_images:
                # Media-only message (image/pdf) - no preamble needed;
                # placeholder caption gets supplied at API-build time if
                # user_text is empty.
        ...
        if threshold > 0:
            ...
            system_prompt_text = _effective_system_prompt(self._config)
        ...
        system_prompt = _effective_system_prompt(self._config)
    ...
    def _run_agent_loop(self, api_key, model_id, system_prompt):
        ...
        for round_idx in range(MAX_AGENT_ROUNDS):
            ...
            # Surface any web-search citations from this round as a
            # "Sources" footer on the turn (verifiable links for code
            # values / specs Claude looked up).
            self._dispatch_citations(_collect_citations(blocks))
        ...
        # Hit MAX_AGENT_ROUNDS. The work done so far is already committed
        # (each tool call committed its own transaction). Do ONE final
        # round with tools suppressed so the model can only summarize -
        # gives the user a clean wrap-up of what got done + what remains,
        # instead of a scary "internal error". History is preserved, so
        # the user can just say "continue" to finish the rest.
        if self._cancelled:
            self._dispatch_cancelled()
            return

        def _quiet_error(label, detail):
            pass  # swallow - we fall back to a static note below
        ...
        if blocks:
            self._dispatch_assistant_turn(blocks, usage)
            # Gentle note so the user knows there may be more to do.
            self._dispatch_error(
                "step_limit",
                "I reached the {}-step limit for one message, so I paused "
                "here. Everything I did above is saved. Say \"continue\" if "
                "there's more to finish.".format(MAX_AGENT_ROUNDS))
        else:
            # Wrap-up round failed - static note only.
            self._dispatch_error(
                "step_limit",
                "I reached the {}-step limit for one message and paused. The "
                "work I completed above is saved in the model. Say "
                "\"continue\" to finish the rest, or break the request into "
                "smaller steps.".format(MAX_AGENT_ROUNDS))
        self._dispatch_finish()
    ...
    def _dispatch_citations(self, citations):
        if not citations:
            return
        self.Dispatcher.BeginInvoke(
            System.Action(lambda c=citations: self._on_citations_ui(c)))

    def _on_citations_ui(self, citations):
        """Append/extend a single 'Sources' panel at the bottom of the
        active turn with clickable links for the web-search results
        Claude cited. Deduped by URL across the whole turn."""
        if not citations:
            return
        _, content = self._ensure_turn_bubble()
        try:
            seen = self._turn_citation_urls
        except AttributeError:
            self._turn_citation_urls = seen = set()
        # Build the panel lazily on first citation.
        if getattr(self, "_turn_sources_panel", None) is None:
            panel = StackPanel()
            panel.Margin = Thickness(0, 8, 0, 0)
            header = TextBlock()
            header.Text       = u"Sources"
            header.Foreground = _BRUSH_BOT_LABEL
            header.FontSize   = 10
            header.FontWeight = FontWeights.SemiBold
            header.Margin     = Thickness(0, 0, 0, 3)
            panel.Children.Add(header)
            content.Children.Add(panel)
            self._turn_sources_panel = panel
        panel = self._turn_sources_panel
        for c in citations:
            url = (c or {}).get("url") or ""
            if not url or url in seen:
                continue
            seen.add(url)
            try:
                panel.Children.Add(self._make_source_link(
                    url, c.get("title") or url))
            except Exception:
                pass
        self._scroll_to_bottom()

    def _make_source_link(self, url, title):
        """A numbered, clickable source line that opens in the browser."""
        ...
        bullet = Run()
        bullet.Text = u"• "
        bullet.Foreground = _BRUSH_BOT_LABEL
        tb.Inlines.Add(bullet)
        link = Hyperlink()
        link_text = title if len(title) <= 90 else (title[:90] + u"...")
        link.Inlines.Add(Run(link_text))
        link.ToolTip = url
        captured = url
        # NavigateUri makes it render/behave as a real link; RequestNavigate
        # fires on click. Only ONE handler (no Click) to avoid opening twice.
        try:
            link.NavigateUri = System.Uri(url)
        except Exception:
            pass
        def _open(s, a, u=captured):
            try:
                Process.Start(u)
            except Exception:
                pass
    ...
    def _on_assistant_turn_ui(self, blocks, usage):
        ...
        if (len(blocks) == 1
            ...
            for b in blocks:
                ...
                # server_tool_use / tool_search_tool_result blocks are
                # NOT persisted: they're server-side artifacts of the
                # tool-search round. Dropping them is safe because the
                # deferred tool's full definition is still in every
                # request's tools array, so the matching tool_use +
                # tool_result pair validates without them.
        # Defensive: never persist a truly empty assistant turn (would
        # 400 the next request). Only possible if a round somehow had
        # only server-side blocks; substitute a no-op space.
